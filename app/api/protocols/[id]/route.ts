...
function transformProtocol(row: Record<string, unknown>) {
  return {
    id: row.id,
    userId: row.user_id,
    ...
    status: row.status,
    version: row.version,
    supersededByProtocolId: row.superseded_by_protocol_id,
...
async function ownedProtocol(id: string, userId: string): Promise<boolean> {
  const rows = await sql`
    SELECT 1 FROM protocols WHERE id = ${id} AND user_id = ${userId} AND deleted_at IS NULL
  `
...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    `
    if (rows.length === 0) return errorResponse(ErrorCodes.NOT_FOUND, 'Protocol not found', 404)
    return successResponse(transformProtocol(rows[0] as Record<string, unknown>))
  } catch (error) {
    console.error('GET /api/protocols/[id] error:', error)
    return errorResponse(ErrorCodes.DATABASE_ERROR, 'Failed to load protocol', 500)
  }
}
...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (!(await ownedProtocol(params.id, userId))) {
      return errorResponse(ErrorCodes.NOT_FOUND, 'Protocol not found', 404)
    }
    const body = await request.json()
    const { title, description, category, status, visibility, projectId, triggerEvent, ruleBody, violationSeverity } = body
    ...
    if (violationSeverity !== undefined && !['info', 'warning', 'error', 'fatal'].includes(violationSeverity)) {
      return errorResponse(ErrorCodes.VALIDATION_ERROR, 'Invalid violationSeverity', 400)
    }
    if (projectId) {
    ...
    const existing = await sql`
      SELECT documentation_5wh, project_id FROM protocols
    ...
    const mergeResult = mergeEnvelopeForPatch(
      ...
      },
      {
        ...
        rationale: body?.documentation_5wh?.why?.rationale || 'Update via PATCH /api/protocols/[id]',
      }
    )
    ...
    const result = await sql`
      UPDATE protocols SET
        ...
        trigger_event     = COALESCE(${triggerEvent ?? null}, trigger_event),
        rule_body         = COALESCE(${ruleBody ?? null}, rule_body),
        violation_severity = COALESCE(${violationSeverity ?? null}, violation_severity),
        documentation_5wh = ${envelopeForSql(mergeResult.envelope)}::jsonb,
        updated_at        = NOW()
      WHERE id = ${params.id} AND user_id = ${userId} AND deleted_at IS NULL
      RETURNING *
    `
    if (result.length === 0) return errorResponse(ErrorCodes.NOT_FOUND, 'Protocol not found', 404)
    return successResponse(transformProtocol(result[0] as Record<string, unknown>))
  } catch (error) {
    console.error('PATCH /api/protocols/[id] error:', error)
    return errorResponse(ErrorCodes.DATABASE_ERROR, 'Failed to update protocol', 500)
  }
}

/** DELETE /api/protocols/[id]  (soft delete) */
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    const authContext = await getAuthContext()
    if (!authContext) return errorResponse(ErrorCodes.UNAUTHORIZED, 'Authentication required', 401)
    const result = await sql`
      UPDATE protocols SET deleted_at = NOW()
      WHERE id = ${params.id} AND user_id = ${authContext.userId} AND deleted_at IS NULL
      RETURNING id
    `
    if (result.length === 0) return errorResponse(ErrorCodes.NOT_FOUND, 'Protocol not found', 404)
    ...
    console.error('DELETE /api/protocols/[id] error:', error)
    return errorResponse(ErrorCodes.DATABASE_ERROR, 'Failed to delete protocol', 500)
  }
}
